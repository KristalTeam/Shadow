...
---
    ...
    Go to [Events](/wiki/using-events) to know about all of the Events that Kristal has.
...
<Box className={styles.layer_table}>

    ## More layers
    ---

    We've already created an objects, markers and collision layer. But if we look into the code that creates a new
    map we can see this code:
    ```lua
    ...
    ```
    This code says that you can create multiples of each layer with different names.

    For example if you want to create an objects layer that only contains things such as scripts or interactables 
    and other objects layer with the slideareas, you can create a layer called `objects_cutscenes` and another called `objects_slideareas`. Both of them will
    behave as objects layer.

    This is the list of **layers that Kristal defines** and the **color you can use** in the layer in both exadecimal and RGB.
    
    You can use the color that you want, but **make them consistent in your project**, don't change the color of the same layer type across different maps.

    |Layer|Description|Color|
    |---|---|---|
    |`objects`|The layer where most events get created.|<span style={{color: "#ff00ff"}}> Pink. </span> #ff00ff <br></br> (255, 0, 255) ![](/wiki/pink_color.png)|
    |`controllers`|Used to create some special events, see the [Event tutorial](/wiki/using-events) to know more.<br></br>I like to use a golden color to make them stand out.|<span style={{color: "#ffc800"}}>Golden. </span>#ffc800<br></br> (255, 200, 0)  ![](/wiki/controllers_layer_color.png)|
    |`markers`|The layer where markers on a map are created, can be used in many ways instead of defining fixed x and y positions. See the [markers tutorial](#markers).|<span style={{color: "#7f00ff"}}> Purple. </span>#7f00ff<br></br> (127, 0, 255)![](/wiki/markers_layer_color.png)|
    |`collision`|The layer where the collision of the map is defined, see the [collision tutorial](#collision).|<span style={{color: "#0000ff"}}>Blue. </span> #0000ff <br></br> (0, 0, 255) ![](/wiki/collision_layer_color.png)|
    |`enemycollision`|A special kind of collision that doesn't affect the player, but affects the ChaserEnemy. A more in-depth tutorial will be created about this.|<span style={{color: "#00ffff"}}>Aqua. </span> #00ffff<br></br> (0, 255, 255)![](/wiki/enemycollision_layer_color.png)|
    |`blockcollision`|A layer that defines a special kind of collision that only affect pushblocks. More to know in that tutorial.|<span style={{color: "#00ffff"}}>Aqua. </span> #00ffff<br></br> (0, 255, 255)![](/wiki/enemycollision_layer_color.png)|
    |`paths`|Where the paths for wandering enemies and other mechanics are defined. Explained in the [Path System tutorial](/wiki/using-events#path-system-tutorial).<br></br> I like to use green for this layer.|<span style={{color: "#00ff00"}}>Green. </span> #00ff00 (0, 255, 0)![](/wiki/paths_layer_color.png)|
    |`battleareas`|Defines areas in the map where an overworld battle gets triggered. Explained in a future WorldBattles tutorial. <br></br> I like to use red because these areas are dangerous to the player.|<span style={{color: "#ff0000"}}>Red.</span> #ff0000 (255, 0, 0)![](/wiki/battleareas_layer_color.png)|
    |`battleborder`|This layer is a **Tile layer, not an Object layer**. And it defines the outlines of the walkable area in an Overworld battle section. Explained in a future WorldBattles tutorial.|None, because it's a **Tile layer**|
