...
<Box>
    ...
    For example if you want yo create an object layer that only has the cutscenes scripts and other with the slideareas,
    you can create a layer called `objects_cutscenes` and another called `objects_slideareas`. Both of them will
    behave as a normal objects layer.

    As we can see, Kristal defines these layers:
    - `objects`: The layer when most events get created.
    - `controllers`: Used to create two special events, see the [Event tutorial](/wiki/using-events) to know more.
    - `markers`: The layer where markers on a map are created, can be used in many ways instead of defining
    fixed x and y positions. See the [markers tutorial](#markers)
    - `collision`: The layer where the collision of the map is defined, see the [collision tutorial](#collision)
    - `enemycollision`: A special kind of collision that doesn't affect the player, but affects the ChaserEnemy.
    A more in-depth tutorial will be created about this
    - `blockcollision`: The collision that only affects the pushblocks. TODO.
    - `paths`: Where the paths for wandering enemies and other mechanics are defined. Explained in the 
    [Path System tutorial](/wiki/using-events#path-system-tutorial).
    - `battleareas`: Defines areas in the map where an overworld battle gets triggered. Explained in a future WorldBattles tutorial.
    - `battleborder` is a Tile layer, not an Object layer. And it defines the outlines of the walkable area in an
    Overworld battle section. Explained in a future WorldBattles tutorial.
