...
Before we get into each debugging tool, we need to make sure they're all **enabled**.

Open up your `mod.json` -- if you see `"dev": true`, then you're good to go. If not, change it to `true` (or add it) and save.

---

One more thing.

Go to Kristal's options menu and head over to the Keyboard Controls. Scroll down until you see these four:
...
As a developer, you'll probably want to add debug information to many of your own objects as well.
...
You'll find everyone over on the Kristal Discord (link at the top of this website!), and can drop any modding issues in the **#dev-help** forum there.
</Box>