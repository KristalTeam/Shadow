...
export const metadata = {
    ...
    description: 'A collection of resources to help you work with waves!',
}
...
- [Movement-based (Blue/Orange) Bullets](#blue-orange-bullets)
...
For example, you can see the project template's dummy file sets this up to select the three waves also included in the project template:
...
<Box>
## Bullet Example: Movement-based Bullets [blue-orange-bullets]

A common pair of bullet behaviours you might want to replicate are the Blue/Orange movement bullets from UNDERTALE. 

Let's see how to give a Custom Bullet this behaviour.

To start, open the file of the custom bullet you want to give this behaviour to, or if you don't have one yet, create a new one in `scripts/battle/bullets` and insert this boilerplate:
```lua
...
We can use `ColorUtils.hexToRGB()` and `setColor()` to set the colour we want onto our bullet within `Bullet:init()` like so:
```lua
function BlueOrangeBullet:init(x, y)
    super.init(self, x, y, "bullets/smallbullet")
...
In practice, our `onDamage` hook will feature one of the below functions depending on which bullet type we are creating:
```lua
-- Blue bullet (deal damage when the soul IS moving)
function BlueOrangeBullet:onDamage(soul)
    if soul:isMoving() then -- If the soul is moving...
        return super.onDamage(self, soul) -- Then run normal damage logic
    end

    -- Return a blank table when we cancel damage
    return {}
end
```
...
function BlueOrangeBullet:onDamage(soul)
    if not soul:isMoving() then -- If the soul isn't moving...
        return super.onDamage(self, soul) -- Then run normal damage logic
    end
...
If we want to heal just one party member, then we can use [`TableUtils.pick(tbl)`](/wiki/api/TableUtils#pick) to select a random party member from `Game.battle.party`, and call `PartyBattler:heal(amount)` on them.
...
We also **need to parent our sprite to another object** for it to appear on the screen:

```lua
Game.battle:addChild(warning_sprite)
```

This adds our sprite as a child of the battle, `Game.battle`. 

Any object could be the parent, but `Game.battle` usually makes the most sense.

An altenative we can use in `Wave` scripts is the `Wave:spawnSprite()` function, which handles parenting for us:
...
Let's say our sprite is the animation of a wind up to an explosion - rather than trying to line up the animation with the explosion via a timer, we can just use the `on_finished` callback to spawn the explosion bullet and remove our sprite:
...
This means creating a new file inside of your project's `scripts/objects` folder, and inserting the following code:
...
If we wanted to make this apply to all battles, we would have to [hook `Encounter` itself](/wiki/hooks) to make it the new default implementation.
