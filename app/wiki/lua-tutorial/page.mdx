...
For this tutorial, we are going to be testing using the [tutorialspoint Online Lua Interpreter](https://www.tutorialspoint.com/compilers/online-lua-compiler.htm).
...
Lua is an easy language to learn, but difficult to master.
...
Type this in the box (if not already there), and press the green "Execute" button at the top. You'll see the output in the box to the right.
Congratulations, you've made your first Lua program!

So, what's happening here? Well, let's break it down in parts.

- `print` - This is a **function** name. Functions are chunks of code which "perform an action". In this case, the `print` function writes whatever you give it into the output area.
- `"Hello, World!"` - This is called a **string**. Strings are a sequence of characters, like a sentence. In this case, it's the text we want to "print". Strings are denoted by quotation marks.
- `()` - These are parentheses. They are used to "call" (run) functions, and to input data into one. In this case, we gave `print` a string, being `"Hello, world!"`.
...
You may have noticed the word "local" before we set a variable for the first time. That is because we are making a **local variable**.

Local variables are only accessible in the "scope" they're in. A scope is just a chunk of code. For example:
...
This will print `nil`, because `hello` is a local variable.
It's only accessible inside of that `if` statement, meaning that `hello` does not exist outside of it.

In this example, we use `if`, which will be explained later.

---

### Global Variables

Global variables are what you get when you don't use `local`. They are accessible from anywhere in the code, and are not limited to a scope.

**Most of the time, you should NOT use these.** You should avoid them as much as possible, and only use them when you HAVE to.

**Try to make all of your variables local if you can.** There are other places to store information (tables) which you will learn of later.
