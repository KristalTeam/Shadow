...
These functions can be distinguished by their `on` or `pre` prefixes and the `sprite` (type `ActorSprite`) argument they receive (except for Battle and World context functions, where they get `battler` and `chara` respectively instead). Rather than modifying `self`, we can modify `sprite` to create special effects!
