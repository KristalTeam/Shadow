...
<Box>
    ...
    ---
    This is an example on how to make a shop that doesn't transition into another place and just uses the shop interface, the vending machines in Chapter 3 are a great
    example on this, so I'll try to recreate them in this.
    ...
    Since we've added a new state to the menu, we need to manage this new state in out code.
    We can use the [`Shop:onStateChange(old, new)`](/wiki/api/Shop#onStateChange), overwrite it in your shop code and
    add the new state.
    ```lua
    ...
    Everything that's left, is creating a way of calling this shop. To do that, I'll add the vending machine sprite and
    a [`Interactable`](/wiki/using-events#interactable) and give it a [`script`](/wiki/using-events#making-scripts).
...
<Box>
    ...
    ## Mini shop UI
    ---
    There is one more thing I want to talk about in this tutorial, [`WorldCutscene:showShop()`](/wiki/api/WorldCutscene#showShop) and
    [`WorldCutscene:hideShop()`](/wiki/api/WorldCutscene#hideShop). These can be used inside of a cutscene to show the money and left space that
    the player has.

    I'll show how to create a cutscene that uses those functions inside of a cutscene. And how to use it in your project.

    This is explained better in the [`cutscenes tutorial`](/wiki/cutscenes). Create a new **\<name\>.lua** file inside of `scripts/world/cutscenes`. **name** is the
    group of the cutscene. Then add a new empty cutscene. I'll name my group as **test** and the cutscene as **shop**.
    ...
    With the cutscene done, I'll add some props to the scene and assign the cutscene to a [`Interactable`](/wiki/using-events#interactable).
