...
<Box>
    ...
    <CollapseBox title="The shop file" id="shop-file">
        ...
        If you don't define an ID, you can use the name of the shop file instead, in this case, that would be `test_shop`.
        ...
        As you can see, I will be using deltarune assets to make this tutorial. Mainly the ones from Seam, I'll not distribute them, 
    ...
    <CollapseBox title="Adding items" id="adding-items">
        ...
        To add a item to the shop, we use the [`self:registerItem(item, options)`](/wiki/api/Shop#registerItem) function. **item** is the item ID of the item we want to sell 
        ...
        `Item` already defines most of these options, but you can use this options table if you need to **overwrite** what the item
        already defines, or define the options that the Item doesn't define already.
    ...
    <CollapseBox title="Shopkeepers" id="shopkeeper-bg-music">
        ...
        **First**, create the assets for the shopkeeper with all the expressions that you want it to have, save them in `assets/sprites/shopkeepers/<name>` 
        ...
        In the text, you can change the current animation of the shopkeeper using the **\[emote:\<animation\>\]** tag. More of that in the example.
        ...
        You can use the code for my shopkeeper as a example.
    ...
    <CollapseBox title="Talking menu" id="talking-menu">
        ...
        The replacements are the talk topics that you choose when you've already talked about something. These are used to expand
        on the talk topic.
...
<Box>
    ...
    ---
    This is a example on how to make a shop that doesn't transition into another place and just uses the shop interface, the vending machines in Chapter 3 are a great
